...
> 这里是 2026-08-20 从仓库内证据(PR 历史、ADR、代码与文档里的工作包引用)重建的全景,
> 用来回答「还剩什么、下一步先做哪个」。
>
...
- **PR #12**:Swift 6.4 下 xunit 少报 155 个测试(运行器报 185,XML 只记 30)却静默通过。
  修复:改数 `<testcase>` 元素 + 运行器报告数与 XML 交叉核对;顺带修掉探针第 7 个缺陷
  (变异丢失可执行位且 `cp -f` 还原不恢复权限)。**应最先合 —— 它堵的是 CI 的静默漏报。**
