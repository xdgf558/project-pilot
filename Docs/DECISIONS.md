...
## ADR-0015:快照仓库的校验和范围、版本策略与写入协议

**状态:** 已采纳(两处经项目所有者 2026-09-03 确认)
**日期:** 2026-09-04
**工作包:** P1-05

### 背景

P1-01 定义了 `Checksum` 类型并留下占位:「计算和验证归 P1-05,它知道实际
写进磁盘的字节」。但 v0.2 原文没有规定**校验和覆盖哪些字节**;
版本策略(`SchemaVersion` 文档)也明确留给本层。

### 决策一:校验和覆盖除 checksum 自身外的整份 envelope

校验和 = FNV-1a(整份 envelope 的规范编码,checksum 字段置零)。

被否掉的备选:只算 payload。理由:元数据(revision / 序号)被位翻转时,
payload-only 静默放过 —— 而 revision 被悄悄改小,乐观并发就被拆掉了,
一个用来发现损坏的机制漏掉最要害的损坏。整份 envelope 还让将来新增的
字段自动纳入校验。

**验证方式**是「解码后按当前规范重编码再哈希」。这依赖「重编码字节稳定」
的前提 —— 它由 P1-01 的往返测试钉死;前提一旦破了,表现是完好文件被
误报损坏,必须先修编码,不能放宽比较。

### 决策二:schemaVersion 比当前新 → 可读不可写

读取成功并标记 `isReadOnly`,写入抛出可操作错误(「数据由更新版本写入,
升级后可写」)。与 P4-03「遇到不认识的状态继续只读」安全降级原则同源。

被否掉的备选:拒绝加载 —— 更「安全」,但把用户锁在自己的数据外面;
数据本身没坏,不该惩罚用户。

### 决策三:写入协议(乐观并发的两道检查)

1. `expected` = 调用方最后一次读到的 revision(首次创建传 `.initial`);
   磁盘上的 revision 与它不一致 —— 包括文件已消失 —— 就抛
   `revisionConflict`,绝不覆盖。
2. 要保存的 envelope.revision 必须恰好是 `expected.next`。这不是风格:
   revision 不前进,两个 writer 的写入就互相不可见,乐观并发名存实亡。
   首次创建同样适用(envelope 从 r0 前进到 r1)。

**损坏的文件拒绝被覆盖**:写入前对磁盘现存数据做同样的校验和验证,
失败即拒。修复是 P1-11 恢复工具的显式动作,不是写入的副作用 ——
悄悄覆盖会销毁现场。

**IO 错误原样透传**(`FileSystemError`),不换类型:数据冲突(重读重放)
和磁盘故障(换盘/清理)是两种完全不同的应对,上层状态机需要区分。

### 代价

- 校验和验证要求每次读取多一次规范重编码 —— 单用户本地数据,量级可忽略。
- 「首次保存也必须 bump revision」对调用方是个容易忘的契约,
  已有专门测试与 `revisionNotAdvanced` 错误信息提示。
...
### 2026-09-04 审查修订(第二轮):三处

复审确认主体修复生效,但新增 1 P1 + 2 P2。

**一、首次保存无法创建尚不存在的父目录(1 P1)。** 上一轮把目录创建留在
writeAtomically(锁内),而旁路锁的 open(O_CREAT) 更早执行 —— 父目录缺失时
直接 ENOENT,首次运行(Application Support/ProjectPilot 尚不存在)必挂。
测试没抓到,因为 makeSubject 总是预建根目录:夹具替实现扫了地。
修正:createDirectory 上移到取锁**之前**(幂等);新增「父目录完全不存在、
含多级中间目录」的双实现回归测试。

**二、JSONValue 整数值的 .number 无法对称往返(1 P2)。** 上一轮把
.integer 与 .number 定义成不相等,却同时让解码把整数值归到 .integer ——
合法的 .number(42) 持久化后不等于自己,payload 匹配、事件去重、重放断言
全会被影子差异咬到。当时的「不等是有意的」论据(「42 与 42.0 字节不同」)
经审查核实是错的:本台编码器把整数值的 Double 就写成整数字面量,
字节根本没有差异。

修正采用值语义(审查建议的方案 b):跨 case 相等,条件是 Double 精确可逆
(`Int64(exactly:)`)—— 所以 .integer(9007199254740993) 与
.number(9007199254740992) **不**相等(数值上它们就是两个数),
.integer(42) 与 .number(42) 相等;哈希按归一化 Double 计算,
满足 Hashable 契约。方案 a(统一规范化、阻止公开构造整数值 .number)
被否:枚举 case 无法限制构造方,「阻止」只能是文档约定 —— 管不住。
**残留限制**:跨编码器的浮点字面量格式差异(42.0 vs 42、指数写法)仍不保真,
那需要原始文本保留,真出现时是独立工作包。

**三、排他锁契约测试被计数器自己的锁掩蔽(1 P2)。** 共享计数器自增
自带 NSLock,把 withExclusiveLock 换成直接调 body,计数照样 512 ——
测试测不出锁被删。修正:改为观测**同时进入 body 的数量**(body 内保持
1ms 可重叠窗口),断言最大重叠数 == 1;无锁实现必然叠出并发。
计数器断言整体删除。探针实测:InMemory 锁变直通后,新测试
2 函数 / 27 用例红 —— 不再被掩蔽。


### 2026-09-04 审查修订(第三轮):三处

复审确认功能路径生效,但并发测试体系仍有两个假绿窗口,CI 也没兑现零警告。

**一、锁契约测试会吞掉竞争错误。** `try? withExclusiveLock` 后只断言
max == 1。把 flock 改成 `LOCK_EX | LOCK_NB`,竞争时返回 EWOULDBLOCK,
`try?` 把错误丢掉,成功执行的 body 依旧不重叠,max == 1 照样绿 ——
协议要求的阻塞语义已经失效。修正:同时断言 body 执行次数 == 8×32、
失败次数 == 0、最大重叠 == 1。探针实测 LOCK_NB 变异 2 函数 / 22 用例红。

**二、内存文件系统会串行化所有无关文件。** 整个实例一把 `transactionLock`,
生产实现按目标 URL 各一把锁文件。两个不同项目的快照在真实文件系统
可以并发,在替身里被强制串行 —— 掩盖 P1-12 跨文件竞态。修正:按标准化
路径建锁表(与生产同构);新增「两条不同目标可以同时进入 body」契约测试
(A 持锁期间 B 必须在 5 秒内进入自己的锁)。探针实测锁表退化成一把全局锁
后 1 函数 / 1 用例红。

**三、CI 全绿但新增测试实际带编译警告。** 规则写「全部 target 零警告」,
但 `swift build` 不编译测试目标,`swift test` 又从没带过 `-warnings-as-errors`
—— 测试代码的警告在 CI 上从来不可见。修正:`run-tests.sh` 给 Debug 与
Release 都加上该参数。本轮被它当场抓出的警告(`Error` 应写 `any Error`、
未使用的 `b`、未标记 `@Sendable`、`try?` 结果未使用)一并修掉。

---

## ADR-0016:事件日志是游标层,不是领域重放器

**状态:** 已采纳
**日期:** 2026-09-07
**偏离:** P1-06 工作包原文与 v0.2 §3 事件文件名均不在仓库内,无从对照
**工作包:** P1-06

### 背景

仓库内关于事件日志的证据已经足够切开存储层,但不够设计「把事件折回
领域对象」:

- `Event` 是不可变审计记录,序号从 1 起,快照靠 `lastEventSequence`
  知道重放到哪(字段文档,P1-02b);
- `CanonicalJSON.makeEventEncoder()` 禁止 prettyPrinted,因为 NDJSON
  要求一条事件占一行(P1-01 / EventTests);
- 旧类型必须永远读得出来(`OpenEnum` / `EventType`);
- ROADMAP 把本包写成「NDJSON、sequence 游标、快照重放起点」。

**没有**命令集、没有「每种 eventType 怎么改 Project / PilotTask」的原文。
按规则第 5 条不猜。P1-03 已经否掉「完整 reducer 骨架」,理由同样适用。

### 决策

**一、本层只持久化和按游标切片。** `load` / `append` / `events(after:)` /
`eventsToReplay(from:log:)`。重放的含义是「从快照游标之后把事件交给
调用方」,不是把 payload 折成领域状态。折回去是 P1-07 / P5-01 的事。

**二、落盘用整文件原子替换,不用 POSIX append。** FileSystem 没有
append 原语。就算加,写到一半崩溃留下的半截最后一行,和「这行本来就坏了」
在读回时无法区分。复用 P1-05 的「临时文件 + replaceItem」:崩溃时旧日志
完好。代价是每次追加重写整份文件 —— 单用户本地审计日志,量级可接受。
fsync 级断电持久性仍归 P1-12。

**三、序号协议与 P1-05 的 revision 同构。** 调用方传入 `expectedLast`
(空日志为 0);磁盘末尾必须等于它;新事件必须从 `expectedLast + 1`
连续前进。保存事务整体处于 `withExclusiveLock` 内。损坏日志拒绝被覆盖。

被否掉的备选:由日志层分配序号。`Event.sequence` 已经是构造期不变量,
调用方必须带着序号来;日志再分配会造出「内存里的序号」和「落盘序号」
两套真相。

**四、游标超过日志末尾是错误,不是「没有新事件」。** 快照说重放到 #10、
日志只到 #5,要么不是一对,要么日志被截断。返回 `[]` 会把这种损坏
假装成「已经追上」。空日志上 `after: 0` 仍返回 `[]`(首次运行)。

**五、单条事件不加校验和。** `Event` 没有这个字段。完整性靠「能按
`Event` 解开 + 序号等于行号」。位翻转若仍能解开且保持序号,本层看不出 ——
记入已知限制,不在这里扩展领域模型。

### 代价

- 领域重放推迟到命令层真正存在之后。在那之前,「快照可以由事件
  重放出来」只兑现了游标,没有兑现折回。
- 整文件重写随日志变长变贵。出现体积问题再做分段 / 压缩,那是
  独立工作包。
...
## ADR-0018:恢复工具是只读诊断报告,不是修复器,也不是界面

**状态:** 已采纳
**日期:** 2026-09-08
**偏离:** P1-11 工作包原文不在仓库内;ADR-0007 写过「界面」二字
**工作包:** P1-11

### 背景

P1-01 把时间戳写成 Double,代价记在 ADR-0007:原始文件人读不懂,
「可读性可以由 P1-11 的恢复工具补上」。P1-05 / P1-06 把损坏文件
拒绝覆盖,修复推到「P1-11 的显式动作」。ROADMAP 把本包写成
「只读打开、错误定位、可读时间戳」。

ADR-0007 用了「界面」这个词。界面在 Phase 8,本阶段没有 Xcode 工程。
按规则第 5 条不把「报告」做成 SwiftUI。

### 决策

**一、产出是 `RecoveryReport`,不是写回。** 只走 `load` / `exists` /
`read`。损坏的覆盖仍是显式修复动作,本包不做 —— 检查的副作用若
改了现场,定位就没了。

**二、错误定位复用仓库层已经给出的坐标。** 快照:`url` + `reason`;
日志:`url` + 行号。本层不重新解析 JSON 去猜列号。

**三、时间戳固定 UTC、毫秒、`Z` 后缀。** 不用本地时区。同一份损坏
在东京和在纽约打开必须对得上。实现用 `ISO8601DateFormatter` 的
`.withInternetDateTime` + `.withFractionalSeconds`。

**四、快照按 `JSONValue` 打开。** 恢复工具不能依赖某个业务 Payload
类型;未知结构照样要能读出外壳上的 revision / 游标 / 时间。

**五、不做 `pilotctl inspect`。** 参数解析仍保持零外部依赖。Phase 8
的界面和以后的 CLI 都渲染同一份 `formattedDescription()`。

被否掉的备选:在检查时自动重算校验和写回;在本包做备份恢复命令
(那是带 reason 的写入,随命令入口落地);用本地时区格式化。

### 代价

- 人要看报告,还得有一个宿主把 `formattedDescription()` 打出来。
  Phase 8 之前只能写测试或临时代码。
- 不做修复,损坏文件仍需从备份恢复。

### 回退条件

v0.2 原文要求本包必须能写回修复,或必须交付 CLI 子命令时,以原文为准。
